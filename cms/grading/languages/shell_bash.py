...
"""shell script programming language, version 3, definition."""
import os
...
class ShellBash(CompiledLanguage):
    """This defines the shelll script programming language, version 18 (more
    precisely, the subversion of shell script available on the system)
    using the default interpeter in the system.

    """

    MAIN_FILENAME = "main.sh"
    ...
    def get_compilation_commands(self,
        ...
        for idx, source_filename in enumerate(source_filenames):
            ...
            sh_filename = "%s.sh" % basename
            # The file with the entry point must be in first position.
            if idx == 0:
                commands.append(["/bin/mv", sh_filename, self.MAIN_FILENAME])
                files_to_package.append(self.MAIN_FILENAME)
            else:
                files_to_package.append(sh_filename)
    ...
    def get_evaluation_commands(
        ...
        commands.append(["/bin/bash", self.MAIN_FILENAME])
        return commands
