...
"""Ruby programming language, version 3, definition."""
import os
...
class Ruby(CompiledLanguage):
    """This defines the Ruby programming language, version 18 (more
    precisely, the subversion of Ruby available on the system)
    using the default interpeter in the system.

    """

    MAIN_FILENAME = "main.rb"
    ...
    def get_compilation_commands(self,
        ...
        for idx, source_filename in enumerate(source_filenames):
            ...
            rb_filename = "%s.rb" % basename
            # The file with the entry point must be in first position.
            if idx == 0:
                commands.append(["/bin/mv", rb_filename, self.MAIN_FILENAME])
                files_to_package.append(self.MAIN_FILENAME)
            else:
                files_to_package.append(rb_filename)
    ...
    def get_evaluation_commands(
        ...
        commands.append(["/usr/bin/ruby", self.MAIN_FILENAME])
        return commands
